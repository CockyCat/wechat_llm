...
func Run() {
	token := config.LoadConfig().TwitterApiToken
	twitterUserId := config.LoadConfig().TwitterUserId
	...
	userTweets, err := user.Tweets(context.Background(), twitterUserId, tweetOpts)
